# this code is more effective than first one but applicable only in case if one person can lend a maximum of one book of one name only because i used sets in this part to increase efficiency.
# by Nitish Bhardwaj
class Library:
    bookdict=dict()
    numdict=dict()
    ...
    def lend(self,lendername,bookname):
        if bookname not in self.booklist:
            ...
            self.bookdict[bookname]=self.bookdict[bookname].union({lendername})
            self.numdict[bookname]-=1
    def add(self,bookname):
        self.bookdict[bookname]=set()
